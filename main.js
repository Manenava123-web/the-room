...
function switchAuth(tab) {
  ...
  document.getElementById('fRegister').style.display = isLogin ? 'none'  : 'block';
  document.getElementById('mtLogin').classList.toggle('active',  isLogin);
  document.getElementById('mtRegister').classList.toggle('active', !isLogin);
  document.getElementById('mTitle').textContent = isLogin ? 'Bienvenido' : 'Crea tu cuenta';
  document.getElementById('mSub').textContent   = isLogin ? 'Accede para reservar tu lugar' : 'Únete a The Room hoy mismo';
  document.getElementById('mSwitch').innerHTML  = isLogin
    ? '¿No tienes cuenta? <a onclick="switchAuth(\'register\')">Regístrate gratis</a>'
    : '¿Ya tienes cuenta? <a onclick="switchAuth(\'login\')">Inicia sesión</a>';
  clearAlert();
}
    ...
    <div style="position:relative">
      ...
      <div class="user-dropdown" id="udrop">
        <div class="udrop-header">
          ...
          ${u.rol !== 'ADMIN' ? `<div class="udrop-creditos" id="creditosBadge">— clases disponibles</div>` : ''}
        </div>
...
function _renderPkgList(containerId, paquetes) {
  ...
  if (!ul) return;
  ul.innerHTML = paquetes.map(p => {
    const labelClases = p.esMensual
      ? `Plan mensual <span class="pkg-row-label">${p.numClases} clases</span>`
      : `${p.numClases} <span class="pkg-row-label">${p.numClases > 1 ? 'clases' : 'clase'}</span>`;
    const featured = p.esMensual ? ' pkg-row-featured' : '';
    const precio   = `$${Number(p.precio).toLocaleString('es-MX')}`;
    const vigencia = p.vigenciaDias <= 15 ? `${p.vigenciaDias} días` : '1 mes';
    return `<li class="pkg-row${featured}">
      <span class="pkg-row-clases">${labelClases}<span class="pkg-row-vigencia">Vigencia: ${vigencia}</span></span>
      <span class="pkg-row-price">${precio}</span>
      <button class="pkg-row-btn" onclick="seleccionarPaquete(${p.id},${p.numClases},${p.precio})">Seleccionar</button>
    </li>`;
  }).join('');
}
...
function procesarPago() {
  ...
  const btn = document.getElementById('pagoBtnPagar');
...
async function _onOpenpayTokenSuccess(response) {
  ...
  try {
    ...
    showToast(
      '¡Pago exitoso!',
      `Se agregaron ${result.clasesAgregadas} clase${result.clasesAgregadas > 1 ? 's' : ''} de ${disc} a tu cuenta.${vence ? ` Válidas hasta el ${vence}.` : ''}`,
      'success'
    );
  } catch(e) {
    alertEl.textContent = e.message;
...
function _renderEfPaquetes() {
  ...
  const renderSection = (titulo, paquetes) => `
    <div class="ef-disc-section">
      <div class="ef-disc-header">${titulo}</div>
      ${paquetes.map(p => {
  ...
  container.innerHTML = renderSection('Indoor Cycling', cycling) + renderSection('Pilates Reformer', pilates);
}
...
function _renderPaquetesAdmin() {
  ...
  const renderSection = (titulo, items) => items.length === 0 ? '' : `
    <div class="gpaq-section">
      <div class="gpaq-section-title">${titulo}</div>
      ${items.map(renderRow).join('')}
    </div>`;

  list.innerHTML = renderSection('Indoor Cycling', cycling) + renderSection('Pilates Reformer', pilates);
}
...
function closeEnCurso() { document.getElementById('enCursoOverlay').classList.remove('show'); }
